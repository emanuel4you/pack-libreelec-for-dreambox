...
NAME=`ls LibreELEC-AMLGX.a*-box.img | sed -e 's/box.img//'`dreamone.img
mv `ls LibreELEC-AMLGX.a*-box.img` $NAME

dd if=./extboot/uboot_dreamone.bin of="${NAME}" conv=fsync,notrunc bs=1 count=440
dd if=./extboot/uboot_dreamone.bin of="${NAME}" conv=fsync,notrunc bs=512 skip=1 seek=1
pigz -9 $NAME
